...
Usedsever usedsever[3][20000];             //先预留500台服务器
...
void predict_server(char * info[MAX_INFO_NUM], char * data[MAX_DATA_NUM], int data_num, char * filename)
{
	...
	// 	for(int j=1;j<maxDate+1;j++)
	// 	{
	// 		if(Data[j][i]>avg[i]*RATIO)
	// 		{
	// 			Data[j][i]=avg[i]*RATIO/2;                
	// 		}
	// 	}
	// }
	...
	for(int i=1;i<predictTime+1;i++)
	{
		...
		{
			float tot=0;
			for(int k=maxDate+i-timeCnt;k<maxDate+i+1;k++)
			{
	...
	// 	}
	// }	
	float TEST[]={0, 1.15,1.57,1,1,1,   1,1,1,2.9,2.1,   1,1,1,1,1,   1,1,1};
    //              +  +  + + +    ~ + + +
	for(int i=0;i<19;i++)
	{
		if(predictList[i][1]!=0)
		{
			predictList[i][1] = (int)((float)(predictList[i][1])*TEST[i]);
			totaltmp +=	predictList[i][1];
		}
	}	
